...
from services.computenode.models import *
...
class ComputeNodeServiceForm(forms.ModelForm):
    ...
    fabricAddress = forms.CharField(required=True)
    hardwareAddress = forms.CharField(required=True)
    ...
    def __init__(self,*args,**kwargs):
        ...
        if self.instance:
            ...
            self.fields['fabricAddress'].initial = self.instance.fabricAddress
            self.fields['hardwareAddress'].initial = self.instance.hardwareAddress
            self.fields['role'].initial = self.instance.role

    def save(self, commit=True):
        self.instance.nodeId = self.cleaned_data.get("nodeId")
        self.instance.hostname = self.cleaned_data.get("hostname")
        self.instance.fabricAddress = self.cleaned_data.get("fabricAddress")
        self.instance.hardwareAddress = self.cleaned_data.get("hardwareAddress")
...
class ComputeNodeServiceAdmin(ReadOnlyAwareAdmin):
    ...
    fieldsets = [(None, {'fields': ['backend_status_text', 'name','enabled','versionNumber','description',"view_url","icon_url",
                                    'nodeId', 'hostname', 'fabricAddress', 'hardwareAddress', 'role'], 'classes':['suit-tab suit-tab-general']})]
